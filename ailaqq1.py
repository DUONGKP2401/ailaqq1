...
class LogicEngineV9:
    ...
    def analyze_and_select(self, issue_id):
        """
        Phân tích lịch sử và chọn ra 3 nhân vật có khả năng vô địch cao nhất.
        """
        if len(self.history) < self.history_min_size:
            return random.sample(ALL_NV_IDS, 3)

        predictions = []
        num_logics_to_run = 15 
        
        if len(self.logics) < num_logics_to_run:
            num_logics_to_run = len(self.logics)

        logic_indices = random.sample(range(len(self.logics)), k=num_logics_to_run)
        
        for i in logic_indices:
            try:
                predictions.append(self.logics[i]())
            except:
                continue

        vote_counts = Counter(predictions)
        top_chars = [char for char, count in vote_counts.most_common(3)]
        
        if len(top_chars) < 3:
            remaining_chars = [c for c in ALL_NV_IDS if c not in top_chars]
            random.shuffle(remaining_chars)
            top_chars.extend(remaining_chars[:3 - len(top_chars)])
...
def generate_dashboard(config, stats, wallet_asset, logs, coin_type, status_message, key_info) -> Panel:
    ...
    stats_table.add_row("Phiên Bản", "LOGIC V9 (Top 3)")
...
def check_issue_result(s, headers, predicted_top3, ki):
    """Kiểm tra kết quả ván đấu. Thắng nếu NV về nhất nằm trong 3 NV đã cược."""
    try:
        response = s.get('https://api.sprintrun.win/sprint/recent_10_issues', headers=headers, timeout=5).json()
        for issue in response['data']['recent_10']:
            if int(issue['issue_id']) == int(ki):
                actual_winner = issue['result'][0]
                return actual_winner in predicted_top3, actual_winner
    except Exception: return None, None
    return None, None
...
def bet_cdtd(s, headers, ki, predicted_top3, Coin, bet_amount, logs):
    """
    Đặt cược vào 3 nhân vật sẽ về nhất bằng cách gửi 3 yêu cầu riêng biệt.
    Số tiền cược sẽ được chia đều cho 3.
    """
    if not predicted_top3:
        add_log(logs, "[bold red]Lỗi: Logic không chọn được NV để cược.[/bold red]")
        return

    # Chia đều số tiền cược cho 3 nhân vật
    bet_amount_per_athlete = round(bet_amount / 3, 8)
    
    if bet_amount_per_athlete <= 0:
        add_log(logs, f"[yellow]Số tiền cược {bet_amount:.8f} quá nhỏ để chia cho 3.[/yellow]")
        return

    successful_bets = []
    # Lặp qua 3 nhân vật đã chọn và đặt cược cho từng người
    for athlete_id in predicted_top3:
        try:
            json_data = {
                'issue_id': int(ki),
                'bet_group': 'winner',  # Sử dụng bet_group 'winner' đã được xác nhận
                'asset_type': Coin,
                'athlete_id': athlete_id, # Đặt cược cho từng nhân vật
                'bet_amount': bet_amount_per_athlete,
            }
            ...
            if response.get('code') == 0 and response.get('msg') == 'ok':
                successful_bets.append(NV.get(athlete_id, str(athlete_id)))
            else:
                log_msg = f"[red]Lỗi cược NV {NV.get(athlete_id, athlete_id)}:[/red] [white]{response.get('msg', 'Không rõ lỗi')}[/white]"
                add_log(logs, log_msg)
        except requests.exceptions.RequestException as e:
            add_log(logs, f"[red]Lỗi mạng khi cược NV {NV.get(athlete_id, athlete_id)}:[/red] [white]{e}[/white]")

    if successful_bets:
        bet_info = f"[green]✓ Đã cược {bet_amount_per_athlete:.4f} {Coin} cho mỗi NV:[/green] [cyan]{', '.join(successful_bets)}[/cyan]"
        add_log(logs, bet_info)
...
def main_cdtd(device_id, key_info):
    ...
    bet_amount0 = get_user_input(f'[cyan]Nhập tổng số {Coin} muốn đặt cho mỗi vòng: [/cyan]', float)
    ...
    logic_engine = LogicEngineV9(state_manager=None)
    ...
    with Live(generate_dashboard(config, stats, asset, logs, Coin, "", key_info), console=console, screen=True, auto_refresh=False) as live:
        while True:
            try:
                ...
                predicted_top3 = logic_engine.analyze_and_select(target_issue_id)
                
                # Hàm bet_cdtd sẽ xử lý việc đặt cược cho 3 nhân vật
                bet_cdtd(s, headers, target_issue_id, predicted_top3, Coin, bet_amount, logs)
                
                # Vòng lặp chờ kết quả
                start_wait_time = time.time()
                while True:
                    result, actual_winner = check_issue_result(s, headers, predicted_top3, target_issue_id)
                    if result is not None: break
                    elapsed = int(time.time() - start_wait_time)
                    
                    predicted_names = [NV.get(p, str(p)) for p in predicted_top3]
                    wait_message = f"⏳ Đợi KQ kì #{target_issue_id}: {elapsed}s. Cược Top 3: [cyan]{', '.join(predicted_names)}[/cyan] với tổng cược [yellow]{bet_amount:,.4f} {Coin}[/yellow]"
                    live.update(generate_dashboard(config, stats, current_asset, logs, Coin, wait_message, key_info), refresh=True)
                    time.sleep(1)

                predicted_names_str = ', '.join([f"'{NV.get(p, p)}'" for p in predicted_top3])
                if result: # THẮNG
                    stats['win'] += 1; stats['streak'] += 1; stats['lose_streak'] = 0
                    stats['max_streak'] = max(stats['max_streak'], stats['streak'])
                    log_msg = (f"[bold green]THẮNG[/bold green] - Cược Top 3 [white]{predicted_names_str}[/white], KQ về '[cyan]{NV.get(actual_winner, actual_winner)}[/cyan]'")
                else: # THUA
                    stats['lose'] += 1; stats['lose_streak'] += 1; stats['streak'] = 0
                    stats['consecutive_loss_counts'][stats['lose_streak']] += 1
                    log_msg = (f"[bold red]THUA[/bold red] - Cược Top 3 [white]{predicted_names_str}[/white], KQ về '[red]{NV.get(actual_winner, actual_winner)}[/red]'")
...
[/bold cyan]
    ...
    console.print(Align.center("[bold gold1]Tool VIP V9 - Chế Độ Cược Top 3 Quán Quân[/bold gold1]\n"))
    time.sleep(3)
